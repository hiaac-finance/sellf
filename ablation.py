...
from main import get_env, get_alg, train, plot_learning, evaluate
...
if __name__ == "__main__":
    ...
    args.add_argument("--ablation_type", type=str, default="beta_2")
    args.add_argument("--seed_id", type=int, default=0)
    args = args.parse_args()

    if args.ablation_type == "beta_2":
        ...
        algo_config = OmegaConf.load(f"configs/sellf_ablation_beta2.yaml")
        ...
        for i, params in enumerate(config.algorithm_param_list):
            ...
            main(config_i)
    elif args.ablation_type == "beta_1":
        seed_list = [
            0
        ]
        if args.seed_id < len(seed_list):
            ...
            # load config
            base_config = OmegaConf.load("configs/base.yaml")
            algo_config = OmegaConf.load(f"configs/sellf_ablation_beta1.yaml")
            config = OmegaConf.merge(base_config, algo_config)
