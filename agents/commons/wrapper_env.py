...
class FairWrapper(gym.Wrapper):
    ...
    def reset(self) -> np.ndarray:
        ...
        self.mu_pred = np.zeros(self.env.state.params.num_groups)
    ...
    def step(self, action, pred = None):
        if pred is None:
            pred = 0
        ...
        if self.mu_type == "error":
            ...
            self.mu_pred = [
                np.mean(np.array(self.y_pred_hist[0]) != np.array(self.a_hist[0])) if len(self.y_pred_hist[0]) > 0 else 1,
                np.mean(np.array(self.y_pred_hist[1]) != np.array(self.a_hist[1])) if len(self.y_pred_hist[1]) > 0 else 1,
            ]
            ...
            for i in range(2):
                ...
                if np.sum(self.y_pred_hist[i]) > 0:
                    self.mu_pred[i] = np.mean([self.a_hist[i][j] for j in range(len(self.y_pred_hist[i])) if self.y_pred_hist[i][j] == 1])
                else:
                    self.mu_pred[i] = 1
            ...
            for i in range(2):
                ...
                self.mu_pred[i] = np.mean(self.y_pred_hist[i]) if len(self.y_pred_hist[i]) > 0 else 1
            self.delta = np.abs(self.mu[0] - self.mu[1])
            self.delta_obs = np.abs(self.mu_obs[0] - self.mu_obs[1])
            self.delta_pred = np.abs(self.mu_pred[0] - self.mu_pred[1])
