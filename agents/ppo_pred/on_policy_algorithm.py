...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            y_obs = np.array([state.y_obs]).astype(np.float32)
            group = np.array([state.group]).astype(np.float32)
            delta = np.array([env.get_attr("delta_pred")[0]]).astype(np.float32)
            rollout_buffer.add(self._last_obs, actions, y_obs, y_pred, group, values, log_probs, rewards, self._last_episode_starts, delta)
            # ----------------------------------------------------------------
