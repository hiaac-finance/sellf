...
from agents.ppo_pred.policy import PredActorCriticPolicy
...
from agents.ppo_pred.on_policy_algorithm import OnPolicyAlgorithm
#from agents.ppo.q_custom_utils import RollingAvg

from config import Config


class PPOPred(OnPolicyAlgorithm):
    """
    ...
    def _setup_model(self) -> None:
        super(PPOPred, self)._setup_model()
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                # penalize advantages based on disparity
                deltas = rollout_data.deltas
                disp = torch.min(
                ...
                # get idx of positive actions
                pos_idx = (rollout_data.actions == 1).nonzero(as_tuple=True)[0]

                with th.no_grad():
                    prob_loan = self.policy.prob_loan(rollout_data.observations[pos_idx])

                pred = self.policy.prob_label(rollout_data.observations[pos_idx])
                pred_loss = F.binary_cross_entropy(pred, rollout_data.y_obs[pos_idx], reduction='none') / prob_loan
    ...
    def learn(
            ...
            tb_log_name: str = "PPOPred",
            eval_log_path: Optional[str] = None,
            reset_num_timesteps: bool = True,
    ) -> "PPOPred":

        return super(PPOPred, self).learn(
