...
@attr.s
class CategoricalCreditDrift(Distribution):
  """A Categorical Distribution."""
  probabilities = attr.ib(validator=[_check_sum_to_one,
                                     _check_nonnegative])

  def __attrs_post_init__(self):
    self.dim = len(self.probabilities)

  def sample(self, rng):
    cred_drift = rng.choice(self.dim, p=self.probabilities)
    ...
    if cred_drift == 0:
      ...
      return 1
    # return rng.choice(self.dim, p=self.probabilities)