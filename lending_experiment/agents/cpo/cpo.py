...
from agents.cpo.optimization_utils.conjugate_gradient import cg_solver
from agents.cpo.torch_utils.distribution_utils import mean_kl_first_fixed
from agents.cpo.optimization_utils.hvp import get_Hvp_fun
from agents.cpo.optimization_utils.line_search import line_search
from agents.cpo.torch_utils.torch_utils import flat_grad, get_device, get_flat_params, normalize, set_params
...
#     # check if there is nan in discounted
#     if torch.isnan(discounted).any():
#         print("discounted has nan")
#         print("vals", vals)
#         print("discount_term", discount_term)
#         print("disc_pows", disc_pows)

#     return discounted

def discount(vals, discount_term):
    """
    Computes the numerically stable discounted sum-to-go.
    R_t = r_t + discount_term * R_{t+1}
    """
    n = vals.size(0)
    ...
    # Iterate from the end of the trajectory to the beginning
    for t in reversed(range(n)):
        running_sum = vals[t] + discount_term * running_sum
        discounted[t] = running_sum
        
    # Check for NaNs, just in case vals itself contained NaN
    # if torch.isnan(discounted).any():
    #     print("discounted has nan")
    #     print("vals", vals)
    #     print("discount_term", discount_term)
...
def compute_advs(actual_vals, exp_vals, discount_term, bias_red_param):
    ...
    # check if there is nan in td_res
    # if torch.isnan(td_res).any():
    #     print("td_res has nan")
    #     print("actual_vals", actual_vals)
    #     print("exp_vals", exp_vals)
    #     print("exp_vals_next", exp_vals_next)
...
class CPO:
    ...
    def __init__(self, policy, value_fun, cost_fun, simulator, max_kl=1e-2, max_val_step=1e-2,
                 ...
                 line_search_max_iter=10, line_search_accept_ratio=0.1, model_path=None,
    ...
    def learn(self, total_timesteps):
        n_episodes = int(total_timesteps // (self.simulator.n_trajectories *
                                            self.simulator.trajectory_len))
    ...
    def train(self, n_episodes):
        ...
        while self.episode_num < n_episodes:
            ...
            #print("trajectory_limits", trajectory_limits)
            ...
            for start, end in zip(trajectory_limits[:-1], trajectory_limits[1:]):
                ...
                # verify if there is NaN in advs
                # if torch.isnan(reward_advs[start:end]).any():
                #     print("iter with nan")
                #     print("rewards", rewards[start:end])
                #     # check if there is nan in rewards
                #     print(torch.isnan(rewards[start:end]).any())
                #     print("state_vals", state_vals[start:end])
                #     print(torch.isnan(state_vals[start:end]).any())

            #print("reward_advs", reward_advs)
...
#             constraint_cost = torch.mean(torch.tensor([disc_costs[start] for start in trajectory_limits[:-1]]))
            ...
            reward_sums = [np.mean(trajectory.rewards) for trajectory in memory]
            cost_sums = [np.mean(np.abs(trajectory.costs)) for trajectory in memory]
    ...
    def update_policy(self, observations, actions, reward_advs, constraint_advs, J_c):
        ...
        #print("imp_sampling", imp_sampling)
        #print("reward_advs", reward_advs)
    ...
    def load_session(self):
        load_path = os.path.join(self.model_path)
