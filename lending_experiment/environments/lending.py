...
from lending_experiment.environments import lending_params, core, multinomial
...
class _CreditShift(core.StateUpdater):
    ...
    def update(self, state, action):
        ...
        assert state.group_id == np.argmax(state.group), 'Group id %s. group %s' % (state.group_id, np.argmax(state.group))
...
class _EnemUpdater(core.StateUpdater):
    """Updates the bank cash based on the action."""
...
class _EnemShift(core.StateUpdater):
    def update(self, state, action):
        params = state.params
        group_id = state.group_id
        cluster_probs = list(
        ...
        # check if is from the last cluster
        if cluster_id == len(cluster_probs) - 1:
            # do nothing
            return
        ...
        if action == LoanDecision.ACCEPT:
            ...
            new_age_group = min(new_age_group, n_age_groups - 1)
            new_cluster_id = new_age_group + school_group * n_age_groups

        mass_to_shift = min(
            params.cluster_shift_increment, cluster_probs[cluster_id]
        )
        cluster_probs[cluster_id] -= mass_to_shift
        cluster_probs[new_cluster_id] += mass_to_shift

        state.params.applicant_distribution.components[group_id].weights = cluster_probs

class EnemEnv(BaseLendingEnv):
    """A variation of the lending environment where individuals are applicants in a test,
    and the objective is to accept applicants that will pass the test. There is a limited number of applicants that can be accepted.
    The reward is the number of accepted applicants that pass the test.
    The dynamics is that with a denial, the chances of passing the test are reduced.
    """

    default_param_builder = lending_params.DelayedImpactParams
    _parameter_updater = _EnemShift()
    def __init__(self, params = None):
        super(EnemEnv, self).__init__(params)
        self.observable_state_vars['applicant_features'] = multinomial.Multinomial(
                self.initial_params.applicant_distribution.dim, 1)
        self.observation_space = spaces.Dict(self.observable_state_vars)
